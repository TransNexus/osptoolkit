...
int
OSPPSecDeleteAuthorityCertificates(
    OSPTSEC         *ospvSecurity);

int
OSPPSecSetDigestAlgorithm(
    OSPTSEC         *ospvSecurity,
    ...
    OSPTSEC **ospvSecurity)
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        ...
        errorcode=OSPPASN1AlgorithmIdEncode(&digestAlgorithm, OSPEID_MD5,
            OSPEDRID_NOTDEFINED);

        /* Set the Digest Algorithm Id */
        if (errorcode == OSPC_ERR_NO_ERROR)
        {
            errorcode=OSPPSecSetDigestAlgorithm(*ospvSecurity,
                digestAlgorithm);
        }
    ...
    if(errorcode != OSPC_ERR_NO_ERROR)
    {
        (void)OSPPSecDelete(ospvSecurity);
    }
    ...
    OSPTPRIVATEKEY *ospvPrivateKey)         /*In - allocated private key store*/
{
    ...
    errorcode = OSPPSecLock(ospvSecurity, OSPE_LOCK_READ);

    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        /* Get pointers to private key data */
        errorcode = OSPPSecGetPrivateKeyData(ospvSecurity,
            &privateKeyData,
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        /* Test data size */
        if ((ospvPrivateKey == OSPC_OSNULL) || 
            (ospvPrivateKey->PrivateKeyData == OSPC_OSNULL) ||
            (privateKeyDataLength > ospvPrivateKey->PrivateKeyLength))
        {
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        ...
        OSPM_MEMCPY(ospvPrivateKey->PrivateKeyData, 
            privateKeyData, privateKeyDataLength);
    ...
    unsigned long   *ospvDeviceId)
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        errorcode = OSPPX509CertGetCustDeviceId(newCertInfo, 
            ospvCustomerId, ospvDeviceId);
    }
        ...
        unsigned *ospvCertificateCount)    /* In/Out - Max allowed/# returned */
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        ...
        for(i=0 ; (errorcode == OSPC_ERR_NO_ERROR) && (i < certCount); i++)
        {
            ...
            errorcode = OSPPX509CertGetCertificate(certInfo,
        ...
        if (errorcode == OSPC_ERR_NO_ERROR)
        {
            ...
            { 
                ...
                OSPM_DBGERRORLOG(errorcode, 
                    "Count > 0, but no authority certificates");
            }
        }
    }
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        ...
        for(count=0;(count<certNum)&&(errorcode==OSPC_ERR_NO_ERROR);count++)
        {
            ...
            if (errorcode == OSPC_ERR_NO_ERROR)
            {
                ...
                certData=NULL;
                errorcode=OSPPX509CertGetCertificate( cert[count],
                    &certData, &certSize);
            }
            ...
            if (errorcode == OSPC_ERR_NO_ERROR)
            {
                ...
                if(ospvRootCACertLen==(int)certSize)
                {
                    ...
                    if (OSPM_MEMCMP(ospvRootCACert,certData,
                        ospvRootCACertLen) == 0)
                    {
                        certMatch=OSPC_TRUE;
                        break;
      ...
      int ospvSignatureOnly)
{
    ...
    */
    ...
    authorities. */
    errorcode = OSPPPKCS7SignatureParse(&signatureObject,
    ...
    {
        errorcode = OSPPPKCS7SignatureGetContent(signatureObject,
            &content, &contentLength,&el);
    }
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        errorcode = OSPPPKCS7SignatureVerify(signatureObject, 
            content,
            contentLength,
            authorityCertInfos,
            authorityCertInfoCount);
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        ...
        if (!ospvSignatureOnly)
        {
            ...
            if((*ospvContent)==OSPC_OSNULL)
            {
                ...
                OSPM_DBGERRORLOG(errorcode, 
                    "Couldn't allocate space for content");
            }
...
/* Copy the DER encoded local certificate from the security object 
    to the storage supplied by the caller */
int 
    ...
    OSPTSEC         *ospvSecurity)
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        ...
        for (   i = 0 ; 
            ospvSecurity->NumberOfAuthorityCertificates;
            i++, ospvSecurity->NumberOfAuthorityCertificates--)
        {
            ...
            if (certInfo == OSPC_OSNULL)
            {
                errorcode = OSPC_ERR_SEC_NULL_CERTIFICATE;
                OSPM_DBGERRORLOG(errorcode, 
                    "Found unexpected NULL certificate");
            }
    ...
    int ospvSignatureOnly)
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        ...
        if (errorcode == OSPC_ERR_NO_ERROR)
        {
            errorcode = OSPPSecSignatureVerify(ospvSecurity,
                &content,
                &contentLength,
                *ospvSignature,
                *ospvSignatureLength,
    ...
    unsigned int  *ospvPrivateKeyDataLength)
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        errorcode = OSPPPKCS8KeyInfoGetPrivateKey(ospvSecurity->PrivateKeyInfo, 
            ospvPrivateKeyData, ospvPrivateKeyDataLength);
    }
    ...
    */
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        ...
        errorcode = OSPPX509CertValidateCertificate(testCert,
            ospvSecurity->AuthorityCertInfo,
            ospvSecurity->NumberOfAuthorityCertificates,
            &parentIndex);
        ...
        if (errorcode == OSPC_ERR_NO_ERROR)
        {
            ...
            *ospvCAIndex = parentIndex;
            ...
            if (localcert)
            {
                errorcode = OSPPX509CertGetCertificate(
                    ospvSecurity->LocalCertInfo,
            ...
            {
                errorcode = OSPPX509CertGetCertificate(
                    ospvSecurity->AuthorityCertInfo[parentIndex], 
                    &certificate, &certificateLength);
            }
            ...
            if (errorcode == OSPC_ERR_NO_ERROR)
            {
                if (*ospvCertificateLength < certificateLength)
                {
                    errorcode = OSPC_ERR_SEC_NOT_ENOUGH_SPACE_FOR_COPY;
                    OSPM_DBGERRORLOG(errorcode, 
                        "Not enough space provided for certificate");
                }
                else
                {
                    OSPM_MEMCPY(ospvCertificate, certificate, 
                        certificateLength);
...
)
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        ...
        errorcode = OSPPASN1ObjectGetElementByDataRef(signercert,
            &eInfo2, 

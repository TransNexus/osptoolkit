    ...
    short ospvPort)
{
    ...
    OSPTSSLSESSIONID *sslsessionid = OSPC_OSNULL, *item = OSPC_OSNULL;
    int result = 1;
    ...
    if (ospvSecurity == OSPC_OSNULL) {
        ...
        OSPM_MALLOC(sslsessionid, OSPTSSLSESSIONID, sizeof(OSPTSSLSESSIONID));
        if (sslsessionid != OSPC_OSNULL) {
            OSPM_MEMSET(sslsessionid, 0, sizeof(OSPTSSLSESSIONID));
            OSPM_MALLOC(sslsessionid->Value, unsigned char, ospvLength);
            ...
            if (errorcode != OSPC_ERR_NO_ERROR ||
                (sslsessionid != OSPC_OSNULL && (sslsessionid->Value == OSPC_OSNULL || sslsessionid->HostName == OSPC_OSNULL))) {
                OSPPSecSSLSessionIdDelete(ospvSecurity, &sslsessionid, OSPC_FALSE);
                errorcode = OSPC_ERR_SSL_INIT_SESSION_FAILED;
            }
        } else {
    ...
    OSPTBOOL ospvUseMutex)
{
    ...
    if (ospvSSLSessionId != OSPC_OSNULL && (*ospvSSLSessionId) != OSPC_OSNULL) {
        if (ospvSecurity->SSLSessionHasMutex == OSPC_TRUE) {
            if (ospvUseMutex) {
                OSPM_MUTEX_LOCK(ospvSecurity->SSLSessionMutex, errorcode);
            }
...
void OSPPSecSSLSessionIdDeleteDB(
    OSPTSEC *ospvSecurity)
{
    OSPTSSLSESSIONID *sslsessionid = OSPC_OSNULL;
    int errorcode = OSPC_ERR_NO_ERROR;
...
unsigned OSPPSecGetSSLLifetime(
    OSPTSEC *ospvSecurity)
{

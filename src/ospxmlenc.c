    ...
    OSPTBFR **ospvBfrAddr)      /* buffer to add to */
{
    ...
    if (errcode == OSPC_ERR_NO_ERROR) {
        errcode = OSPPXMLDocAddElemName(ospvElem, ospvBfrAddr);
    }
    ...
    if (errcode == OSPC_ERR_NO_ERROR) {
            ...
            attr = (OSPT_XML_ATTR *)OSPPXMLElemNextAttr(ospvElem, attr))
        {
             ...
             */
            errcode = OSPPXMLDocAddChar(ospvBfrAddr, OSPC_XMLDOC_SPACE);

            /* now add the attributes themselves */
            if (errcode == OSPC_ERR_NO_ERROR) {
                errcode = OSPPXMLDocAddAttr(attr, ospvBfrAddr);
            }
        }
    }
    ...
    if (errcode == OSPC_ERR_NO_ERROR) {
        ...
        if ((child == OSPC_OSNULL) && (vallen == 0)) {
            ...
            errcode = OSPPXMLDocAddChar(ospvBfrAddr, OSPC_XMLDOC_TRAILER);
        }
    }

    /**** STEP 5: add the closing bracket (for the opening tag) */
    if (errcode == OSPC_ERR_NO_ERROR) {
        errcode = OSPPXMLDocAddChar(ospvBfrAddr, OSPC_XMLDOC_CLOSE);
        /* add newline for readability */
        errcode = OSPPXMLDocAddChar(ospvBfrAddr, OSPC_XMLDOC_LF);
    }
    ...
    if (errcode == OSPC_ERR_NO_ERROR) {
        ...
        for (child = (OSPT_XML_ELEM *)OSPPXMLElemFirstChild(ospvElem);
            ((child != OSPC_OSNULL) && (errcode == OSPC_ERR_NO_ERROR));
            child = (OSPT_XML_ELEM *)OSPPXMLElemNextChild(ospvElem, child))
        {
             ...
             */
            errcode = OSPPXMLDocAddElem(child, ospvBfrAddr);
        }
    }
    ...
    OSPTBFR **ospvBfrAddr)      /* buffer to add to */
{
    ...
    if (errcode == OSPC_ERR_NO_ERROR) {
        ...
        errcode = OSPPXMLDocAddAttrName(ospvAttr, ospvBfrAddr);
    }
    /* then the equals sign */
    if (errcode == OSPC_ERR_NO_ERROR) {
        errcode = OSPPXMLDocAddChar(ospvBfrAddr, OSPC_XMLDOC_EQUALS);
    }

    /* and the quotation mark */
    if (errcode == OSPC_ERR_NO_ERROR) {
        errcode = OSPPXMLDocAddChar(ospvBfrAddr, OSPC_XMLDOC_QUOTE);
    }

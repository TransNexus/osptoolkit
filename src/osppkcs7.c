...
#define DUMPOBJECTDATA(a, b) { \
    unsigned char *_data; \
    unsigned int _dataLength; \
    OSPTASN1ELEMENTINFO *_eInfo; \
    OSPPASN1ObjectGetElementInfo(a, &_eInfo); \
    OSPPASN1ElementGetElementData(_eInfo, &_data, &_dataLength); \
    OSPTNLOGDUMP(_data, _dataLength, b); \
}
    ...
    unsigned *ospvSignatureLength)          /* Out - length of signature */
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        errorcode = OSPPPKCS7ContentInfoCreate(&contentObject, dataContent, OSPEID_DATA, ospvSignatureOnly);
    }

    if (errorcode == OSPC_ERR_NO_ERROR) {
        /* Generate the Set of Certificates */
        errorcode = OSPPPKCS7CertificatesCreate(&certificates, ospvSignerCertInfo);
    }

    if (errorcode == OSPC_ERR_NO_ERROR) {
        /* Generate the set of DigestAlgorithms */
        errorcode = OSPPPKCS7DigestAlgorithmsCreate(&digestAlgorithms, ospvDigestAlgorithm);
    }
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        errorcode = OSPPPKCS7SignerInfoCreate(&signerInfo, dataContent, ospvDigestAlgorithm, ospvSignerCertInfo, ospvSignerPrivateKey);
    }

    if (errorcode == OSPC_ERR_NO_ERROR) {
        /* Generate the Set of Signer Info structures. */
        errorcode = OSPPPKCS7SignerInfosCreate(&signerInfos, signerInfo);
    }

    if (errorcode == OSPC_ERR_NO_ERROR) {
        /* First, create the signed data object using the function parameters. */
        errorcode = OSPPPKCS7SignedDataCreate(&signedData, contentObject, digestAlgorithms, certificates, signerInfos);
    }

    if (errorcode == OSPC_ERR_NO_ERROR) {
        /* Next, create a ContentInfo with type SignedData. */
        errorcode = OSPPPKCS7ContentInfoCreate(&contentInfo, signedData, OSPEID_SIGNEDDATA, 0);
    }

    if (errorcode == OSPC_ERR_NO_ERROR) {
        errorcode = OSPPASN1ObjectGetElementByDataRef(contentInfo, &eInfo, OSPEDRID_CONTENTINFO);

        if (errorcode == OSPC_ERR_NO_ERROR) {
            errorcode = OSPPASN1ElementCopyElementData(eInfo, ospvSignature, ospvSignatureLength);
        }
    }
    ...
    unsigned int ospvContentLength)
{
    ...
    for (i = 0; errorcode == OSPC_ERR_NO_ERROR; i++) {
        ...
        case 1: /* Add Digest */
            ...
            errorcode = OSPPCryptoDigest(&newObject, ospvDigestAlgorithm, ospvContent, ospvContentLength);
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            ...
            if (newObject != OSPC_OSNULL) {
                errorcode = OSPPASN1ObjectAddChild(digestInfo, newObject, dataRefId);
                OSPM_FREE(newObject);
    ...
    OSPTASN1OBJECT *ospvSignerPrivateKey)
{
    ...
    for (i = 0; errorcode == OSPC_ERR_NO_ERROR; i++) {
        ...
        case 6: /* Add EncryptedDigest */
            ...
            if (errorcode == OSPC_ERR_NO_ERROR) {
                errorcode = OSPPASN1ElementGetContentData(eInfo, &contentData, &contentDataLength);
            }

            if (errorcode == OSPC_ERR_NO_ERROR) {
                errorcode = OSPPPKCS7DigestInfoCreate(&digestInfo, ospvDigestAlgorithm, contentData, contentDataLength);
            }
...
*/
            if (errorcode == OSPC_ERR_NO_ERROR) {
                errorcode = OSPPCryptoEncrypt(&newObject, digestEncryptionAlgorithm, digestInfo, ospvSignerPrivateKey);
            }
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            /* Add new object to this object */
            if (newObject != OSPC_OSNULL) {
                errorcode = OSPPASN1ObjectAddChild(signerInfo, newObject, dataRefId);
                if (newObject != OSPC_OSNULL) {
                    OSPM_FREE(newObject);
                }
            }
    ...
    OSPTASN1OBJECT *ospvDigestAlgorithm)
{
    ...
    errorcode = OSPPASN1ObjectNew(&digestAlgorithms, OSPEDRID_SGNDAT_DIGESTALGORITHMS);
    ...
    for (i = 0; errorcode == OSPC_ERR_NO_ERROR; i++) {
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            /* Add new object to this object */
            if (newObject != OSPC_OSNULL) {
                errorcode = OSPPASN1ObjectAddChild(digestAlgorithms, newObject, dataRefId);
                OSPM_FREE(newObject);
            }
        }
    }
    ...
    OSPTASN1OBJECT *ospvCertificate)
{
    ...
    errorcode = OSPPASN1ObjectNew(&certificates, OSPEDRID_SGNDAT_CERTIFICATES);
    ...
    for (i = 0; errorcode == OSPC_ERR_NO_ERROR; i++) {
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            /* Add new object to this object */
            if (newObject != OSPC_OSNULL) {
                errorcode = OSPPASN1ObjectAddChild(certificates, newObject, dataRefId);
                OSPM_FREE(newObject);
            }
        }
    }
    ...
    OSPTASN1OBJECT *ospvSignerInfo)
{
    ...
    for (i = 0; errorcode == OSPC_ERR_NO_ERROR; i++) {
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            /* Add new object to this object */
            if (newObject != OSPC_OSNULL) {
                errorcode = OSPPASN1ObjectAddChild(signerInfos, newObject, dataRefId);
                if (newObject != OSPC_OSNULL) {
                    OSPM_FREE(newObject);
                }
            }
    ...
    OSPTASN1OBJECT *ospvSignerInfos)
{
    ...
    for (i = 0; errorcode == OSPC_ERR_NO_ERROR; i++) {
        ...
        case 0:
            ...
            errorcode = OSPPASN1SmallIntegerEncode(&newObject, OSPC_PKCS7_SIGNEDDATA_VERSION, dataRefId);
            break;

        case 1:
            dataRefId = OSPEDRID_SGNDAT_DIGESTALGORITHMS;
            errorcode = OSPPASN1ObjectCopy(&newObject, ospvDigestAlgorithms);
            if (newObject != OSPC_OSNULL) {
                OSPPASN1ElementDelete(&(newObject->ElementInfo), 0);
            }
            break;
        ...
        case 3:
            ...
            errorcode = OSPPASN1ObjectCopy(&newObject, ospvSignerCertificates);
            if (newObject != OSPC_OSNULL) {
                OSPPASN1ElementDelete(&(newObject->ElementInfo), 0);
            }
            break;
        ...
        case 4:
            ...
            errorcode = OSPPASN1ObjectCopy(&newObject, ospvSignerInfos);
            if (newObject != OSPC_OSNULL) {
                OSPPASN1ElementDelete(&(newObject->ElementInfo), 0);
            }
            break;
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            /* Add new object to this object */
            if (newObject != OSPC_OSNULL) {
                errorcode = OSPPASN1ObjectAddChild(signedData, newObject, dataRefId);
                OSPM_FREE(newObject);
            }
        }
    }
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        errorcode = OSPPASN1ObjectDeparse(signedData, OSPEPTID_SIGNEDDATA, OSPEDRID_SIGNEDDATA);
    }
    if (errorcode == OSPC_ERR_NO_ERROR) {
    ...
    int ospvSignatureOnly)
{
    ...
    for (i = 0; errorcode == OSPC_ERR_NO_ERROR; i++) {
        ...
        case 0:
            ...
            errorcode = OSPPASN1ObjectIdentifierEncode(&newObject, ospvContentTypeId, dataRefId);
            break;
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            /* Add new object to this object */
            if (newObject != OSPC_OSNULL) {
                errorcode = OSPPASN1ObjectAddChild(contentInfo, newObject, dataRefId);
                OSPM_FREE(newObject);
            }
        }
    }
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        if (ospvContentTypeId == OSPEID_DATA) {
            ...
            OSPM_DBGERRORLOG(errorcode, "Unsupported contentInfo type specified");
        }

        if (errorcode == OSPC_ERR_NO_ERROR) {
            errorcode = OSPPASN1ObjectDeparse(contentInfo, tableId, OSPEDRID_CONTENTINFO);
        }
    }
    ...
    unsigned int ospvSignatureLength)
{
    ...
    if ((ospvSignatureObject == OSPC_OSNULL) || (ospvSignature == OSPC_OSNULL) || (ospvSignatureLength == 0)) {
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        errorcode = OSPPASN1ElementParse(eInfo, OSPEPTID_CONTENTINFO_SIGNEDDATA, OSPC_OSNULL, &parseResults, OSPC_ASN1_DATAREFID_CONTENTINFO);
    }
    ...
    OSPTASN1ELEMENTINFO **el)
{
    ...
    errorcode = OSPPASN1ObjectGetElementByDataRef(ospvSignatureObject, &contentElement, OSPEDRID_SIG_SGNDAT_DATA);
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        errorcode = OSPPASN1ElementGetContentData(contentElement, &content, &contentLength);
    ...
    unsigned int ospvNumberOfAuthorityCertificates)
{
    ...
    if ((ospvContent == OSPC_OSNULL) || (ospvContentLength == 0)) {
        errorcode = OSPC_ERR_PKCS7_INVALID_POINTER;
        OSPM_DBGERRORLOG(errorcode, "Content buffer pointer is null, or length is 0");
    }
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            ...
            errorcode = OSPPASN1ObjectCopyElementObject(&digestAlgorithmId, ospvSignatureObject, OSPEDRID_SIG_SGNDAT_SGNINF_DIGESTALGORITHM);
        }

        if (errorcode == OSPC_ERR_NO_ERROR) {
            /* Create a local Digest Info object to compare to results of
               decryption. */
            errorcode = OSPPPKCS7DigestInfoCreate(&digestInfo, digestAlgorithmId, ospvContent, ospvContentLength);

            if (errorcode == OSPC_ERR_NO_ERROR) {
                errorcode = OSPPASN1ObjectGetElementInfo(digestInfo, &eInfo);
                if (errorcode == OSPC_ERR_NO_ERROR) {
                    errorcode = OSPPASN1ElementGetElementData(eInfo, &contentDigest, &contentDigestLength);
                }
            }
        }
    }
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            ...
            errorcode = OSPPASN1ObjectCopyElementObject(&encryptedDigest, ospvSignatureObject, OSPEDRID_SIG_SGNDAT_SGNINF_ENCRYPTEDDIGEST);
        }
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            ...
            errorcode = OSPPASN1ObjectCopyElementObject(&subjPubKeyInfo, ospvSignatureObject, OSPEDRID_SIG_SGNDAT_CERTIFICATE_SUBJPUBKEYINFO);
        }

        if (errorcode == OSPC_ERR_NO_ERROR) {
            /* Have the public key data */
            errorcode = OSPPCryptoDecrypt(&decryptedDigestObject, encryptionAlgorithm, encryptedDigest, subjPubKeyInfo);
        }
    }
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            errorcode = OSPPASN1ElementGetContentData(eInfo, &decryptedDigest, &decryptedDigestLength);
        }
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            ...
            if (decryptedDigestLength == contentDigestLength) {
                if (OSPM_MEMCMP(decryptedDigest, contentDigest, contentDigestLength) == 0) {
                    errorcode = OSPC_ERR_NO_ERROR;
                }
            }
        }
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        errorcode = OSPPASN1ObjectCopyElementObject(&signerCertificate, ospvSignatureObject, OSPEDRID_SIG_SGNDAT_CERTIFICATE);
        if (errorcode == OSPC_ERR_NO_ERROR) {
            int ix = 0;

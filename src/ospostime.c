 ...
 */
OSPTTIME OSPPOSTimeConvertToGMT(
 ...
 */
unsigned OSPPOSTimeCalToString( /* returns error code (0 if no error) */
 ...
 */
unsigned OSPPOSTimeStringToCal( /* returns error code (0 if no error) */
    ...
    OSPTTIME *ospvCalTime)      /* where to put calendar time */
{
    ...
    year = (ospvTimeString[0] - '0') * 1000 + (ospvTimeString[1] - '0') * 100 + (ospvTimeString[2] - '0') * 10 + (ospvTimeString[3] - '0');
    ...
    char *ospvTimeString)
{
    ...
    if ((ospvFormat == OSPC_OSNULL) || (ospvTimeString == OSPC_OSNULL)) {
        errorcode = OSPC_ERR_INVALID_POINTER;
        OSPM_DBGERRORLOG(errorcode, "Invalid pointer to format or output string");
    }

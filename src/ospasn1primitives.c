    ...
    OSPTASN1BUFFER **ospvBuffer)
{
    ...
    if ((ospvBuffer != OSPC_NULL) && 
        (*ospvBuffer != OSPC_OSNULL) && 
        ((*ospvBuffer)->Buffer != OSPC_OSNULL))
    {
    ...
    unsigned int ospvEncodedDataLength)
{
    ...
    {
        errorcode = OSPPASN1BufferCreate(ospvEncodedData, ospvEncodedDataLength,
            ospvOctetStringBuffer);
    }
    ...
    OSPTASN1INTEGER **ospvInteger)
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR)
    {
        ...
        {
            for (i = 0 ; 
                (i < intBuf->DigitCount) && (smallValue <= 0x7FFFFF) ; i++)
            {
                smallValue *= 256;
            ...
            *****************************************/
                ...
                OSPTASN1ELEMENTINFO *ospvElementInfo)
            {
                    ...
                    OSPTASN1BITSTRING **ospvBitString)
                {
                    ...
                    {
                        /* An error occured, get rid of allocated stuff - don't update 
                            ospvBitString ptr */
                        OSPM_FREE(bitString);
                    }
                    ...
                    OSPEASN1DATAREFID ospvDataRefId)
                {
                    ...
                    errorcode = OSPPASN1ElementFormat(&eInfo, &tag, OSPC_BER_MODE_PRIMITIVE, 
                        1, ospvData, ospvDataLength);
                    ...
                    OSPEASN1DATAREFID ospvDataRefId)
                {
                    ...
                    errorcode = OSPPASN1SmallInt2UnsignedChar(ospvInteger,  256,
                        &tmpData, &tmpLength);

                    if (errorcode == OSPC_ERR_NO_ERROR)
                    {
                        errorcode = OSPPASN1ObjectEncode(ospvEncodedObject, &tag, 1, 
                            OSPC_BER_MODE_PRIMITIVE, 
                            tmpData, tmpLength, ospvDataRefId);
                    }
                    ...
                    int *ospvValue)
                {
                    ...
                    if (errorcode == OSPC_ERR_NO_ERROR) 
                    {
                        ...
                        *ospvValue = 0;
                        ...
                        {
                            for (i = 0 ; 
                                (i < digitCount) && (smallValue <= 0x7FFFFF) ; i++)
                            {
                                smallValue *= 256;
                                smallValue += digits[i];
                            }
                    ...
                    OSPEASN1DATAREFID   ospvDataRefId)
                {
                    ...
                    for (i = 0 ;errorcode == OSPC_ERR_NO_ERROR ; i++)
                    {
                        ...
                        if (errorcode == OSPC_ERR_NO_ERROR)
                        {
                            ...
                            {
                                errorcode = OSPPASN1ObjectAddChild( encodedObject, newObject,
                                    dataRefId );
                                OSPM_FREE(newObject); 
                    ...
                    if (errorcode == OSPC_ERR_NO_ERROR)
                    {
                        /* Complete the encoding for this object. Update results, elements, 
                            etc. */
                        errorcode = OSPPASN1ObjectDeparse(encodedObject, OSPEPTID_ALGORITHMID,
                ...
                *****************************************/
                ...
                {   
                    ...
                    if (errorcode == OSPC_ERR_NO_ERROR)
                    {
                        errorcode = OSPPASN1ObjectFormat(&encodedObject, eInfo, 
                            OSPEDRID_NOTDEFINED);
                    }

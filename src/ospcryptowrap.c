    ...
    unsigned char ospvFlags)
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        if (ospvDigest == OSPC_OSNULL) {
            ...
            OSPM_DBGERRORLOG(errorcode, "Error allocating space for message digest");
        }

        if (*ospvDigestLength < digestedDataLength) {
            errorcode = OSPC_ERR_CRYPTO_NOT_ENOUGH_SPACE;
            OSPM_DBGERRORLOG(errorcode, "Not enough space for digest");
        }

        if (errorcode == OSPC_ERR_NO_ERROR) {
            OSPM_MEMCPY(ospvDigest, digestedData, digestedDataLength);
    ...
    unsigned char ospvFlags)
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        if (ospvDecryptedData == OSPC_OSNULL) {
            ...
            OSPM_DBGERRORLOG(errorcode, "Error allocating space for decrypted data");
        }
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            OSPM_MEMCPY(ospvDecryptedData, decryptedData, decryptedDataLength);

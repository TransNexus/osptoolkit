    ...
    OSPTASN1OBJECT *ospvSignerPublicKey)
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        errorcode = OSPPASN1ElementGetElementData(elementInfo, &certificate, &certificateLength);
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            ...
            errorcode = OSPPASN1ObjectGetElementByDataRef(certInfo, &elementInfo, OSPEDRID_CERT_TBSCERTIFICATE);

            if (errorcode == OSPC_ERR_NO_ERROR) {
                errorcode = OSPPASN1ElementGetElementData(elementInfo, &tbsCertificate, &tbsCertificateLength);
            }
        }

        if (errorcode == OSPC_ERR_NO_ERROR) {
            /* Get the signature from certificate */
            errorcode = OSPPASN1ObjectCopyElementObject(&signature, certInfo, OSPEDRID_CERT_SIGNATURE);
        }

        if (errorcode == OSPC_ERR_NO_ERROR) {
            errorcode = OSPPCryptoVerify(signature, ospvSignerPublicKey, tbsCertificate, tbsCertificateLength);
        }
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        errorcode = OSPPOSTimeGetTime(&currentTime, OSPC_OSNULL);

        if (errorcode == OSPC_ERR_NO_ERROR) {
            /* Format the current time in YYYYMMDDhhmmssZ format */
            errorcode = OSPPOSTimeFormatGMTTime(currentTime, "%Y%m%d%H%M%SZ", currentDate);
        }
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            ...
            errorcode = OSPPASN1ObjectGetElementByDataRef(certInfo, &elementInfo, OSPEDRID_CERT_NOTBEFORE);

            if (errorcode == OSPC_ERR_NO_ERROR) {
                errorcode = OSPPASN1ElementGetContentData(elementInfo, &date, &dateLength);
                if (errorcode == OSPC_ERR_NO_ERROR) {
                    if (dateLength != 13) {
                        errorcode = OSPC_ERR_X509_INVALID_DATE;
                        OSPM_DBGERRORLOG(errorcode, "Date is not 13 characters long");
                    } else {
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            ...
            errorcode = OSPPASN1ObjectGetElementByDataRef(certInfo, &elementInfo, OSPEDRID_CERT_NOTAFTER);
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            ...
            if ((OSPM_MEMCMP(currentDate, notBeforeString,
                             OSPM_STRLEN(currentDate)) < 0) || (OSPM_MEMCMP(currentDate, notAfterString, OSPM_STRLEN(currentDate)) > 0)) {
                errorcode = OSPC_ERR_X509_CERTIFICATE_EXPIRED;
#ifdef IGNOREOUTOFDATEERR
                errorcode = OSPC_ERR_NOERROR;
#endif
                OSPM_DBGERRORLOG(errorcode, "Current date is outside certificate validity dates");
            }
        }
    }
    ...
    int *ospvParentCertificateIndex)
{
    ...
    for (i = 0; (errorcode == OSPC_ERR_NO_ERROR || errorcode == OSPC_ERR_X509_CA_NOT_FOUND) && (i < ospvNumberOfAuthorityCertificates); i++) {
        /* Test ca certificate against test certificate */
        errorcode = OSPPX509CertIsParentCertificate(ospvAuthorityCertificates[i], ospvTestCertificate);
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        errorcode = OSPPASN1ObjectCopyElementObject(&publicKey, ospvAuthorityCertificates[i], OSPEDRID_CERT_SUBJPUBKEYINFO);
    }

    if (errorcode == OSPC_ERR_NO_ERROR) {
        /* Check the test certificate to see if it is signed/not expired */
        errorcode = OSPPX509CertCheckCertificateData(ospvTestCertificate, publicKey);
    }

    if (errorcode == OSPC_ERR_NO_ERROR) {
        /* Is authority certificate self signed? */
        if (OSPPX509CertIsParentCertificate(ospvAuthorityCertificates[i], ospvAuthorityCertificates[i])) {
            /* Yes - is is a Self Signed Certificate */
            /* Check ca certificate (self signed) to make sure it is valid */
            errorcode = OSPPX509CertCheckCertificateData(ospvAuthorityCertificates[i], publicKey);
        } else {
            ...
            if (ospvParentCertificateIndex == OSPC_OSNULL) {
                ...
                errorcode = OSPPX509CertValidateCertificate(ospvAuthorityCertificates[i],
                                                            ospvAuthorityCertificates, ospvNumberOfAuthorityCertificates, OSPC_OSNULL);
            }
        }
    }
    ...
    OSPTASN1OBJECT *ospvTestCertificate)
{
    ...
    errorcode = OSPPASN1ObjectGetElementInfo(ospvParentCertificate, &elementInfo1);

    if (errorcode == OSPC_ERR_NO_ERROR) {
        /* Reparse the certificate */
        errorcode = OSPPASN1ElementGetElementData(elementInfo1, &certificate, &certificateLength);
        ...
        if (errorcode == OSPC_ERR_NO_ERROR) {
            errorcode = OSPPASN1ObjectGetElementInfo(ospvTestCertificate, &elementInfo1);
        }
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        errorcode = OSPPASN1ObjectGetElementByDataRef(parentCertInfo, &elementInfo1, OSPEDRID_CERT_SUBJECT);
        if (errorcode == OSPC_ERR_NO_ERROR) {
            errorcode = OSPPASN1ElementGetElementData(elementInfo1, &subjectName, &subjectNameLength);
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        ...
        errorcode = OSPPASN1ObjectGetElementByDataRef(testCertInfo, &elementInfo2, OSPEDRID_CERT_ISSUER);
        if (errorcode == OSPC_ERR_NO_ERROR) {
            errorcode = OSPPASN1ElementGetElementData(elementInfo2, &issuerName, &issuerNameLength);
        }
    }
    ...
    unsigned int *ospvCertificateLength)
{
    ...
    *ospvCertificateLength = 0;
    ...
    errorcode = OSPPX509CertGetElement(ospvCertInfo, OSPEDRID_CERTIFICATE, &eInfo);
    ...
    OSPTASN1ELEMENTINFO **ospvElementInfo)
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        errorcode = OSPPASN1ElementGet(ospvDataRefId, ospvCertInfo->ParseResults, &foundElement);
    }
    ...
    OSPTASN1OBJECT **ospvCert)
{
    ...
    if (*ospvCert == OSPC_OSNULL) {
        ...
        OSPM_DBGERRORLOG(errorcode, "malloc of new X509 Certificate failed");
    } else {
        OSPM_MEMSET(*ospvCert, 0, sizeof(OSPTASN1OBJECT));
    }

    if (errorcode == OSPC_ERR_NO_ERROR) {
        errorcode = OSPPX509CertSetCertificate(*ospvCert, ospvEncodedCertificate);
    }
    ...
    unsigned char *ospvEncodedCertificate)
{
    ...
    if (errorcode == OSPC_ERR_NO_ERROR) {
        errorcode = OSPPASN1ElementParse(eInfo, OSPEPTID_CERTIFICATE, OSPC_OSNULL, &parseResults, OSPC_ASN1_DATAREFID_CERTIFICATE);
    }

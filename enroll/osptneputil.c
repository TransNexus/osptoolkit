...
void OSPPDumpHex(unsigned char *ospvDumpee, unsigned ospvDumpeeLen)
{
    ...
    if ((ospvDumpee == NULL) || (ospvDumpeeLen <= 0)) {
        ...
        for (lineIndex = 0; lineIndex < ospvDumpeeLen; lineIndex += OSPC_ENROLL_HEX_CHARS_PER_LINE) {
 ...
 */
void OSPPDumpHexLine(unsigned char *ospvDumpeeLine,
    unsigned ospvDumpeeLineLen)
{
    ...
    if ((ospvDumpeeLine != OSPC_OSNULL) && (ospvDumpeeLineLen > 0)) {
         ...
         */
        for (charIndex = 0;
             charIndex < OSPC_ENROLL_HEX_CHARS_PER_LINE; charIndex++) {

...
int main ( int argc, char* argv[] )
{
    ...
    /* The status of an enrollment: 0 for success, 1 for pending, 
     * >= 2 for failure:
     */
    ...
    if ( retVal == OSPC_ERR_NO_ERROR )
    {
         ...
         */
        if ( ( enrollParams.Function == OSPC_OSNULL ) ||  
             ( OSPM_STRLEN( (const char*)enrollParams.Function ) <= 0 ) )
        {
    ...
    /* Now execute whatever function the user entered, if it's a valid
     * operation request.
     */
    if ( retVal == OSPC_ERR_NO_ERROR )
    {
         ...
         */
        if ( OSPM_STRCMP(
                  (const char*)enrollParams.Function,
                  OSPC_ENROLL_CA_CERT_FUNCTION )
                  == 0 )
        {
            ...
            {
                OSPM_DBGERRORLOG( 
                    retVal, "Unable to retrieve the CA certificate\n" );
            }
            else
            {
         ...
         */
            ...
            ( OSPM_STRCMP( 
                  (const char*)enrollParams.Function, 
                  OSPC_ENROLL_REQUEST_FUNCTION ) 
                  == 0 ) 
            ||
            ( OSPM_STRCMP( 
                  (const char*)enrollParams.Function, 
                  OSPC_ENROLL_RETRIEVE_FUNCTION ) 
                  == 0 ) 
                )
        {
            retVal = 
        ...
        {
            OSPM_DBGMISC(( 
                "Invalid function parameter passed in: <%s>\n", 
...
)
{
     ...
     *      - record an error.
     */
    if ( ( ospvStringIn != OSPC_OSNULL ) &&
         ( ospvStringLowercaseOut != OSPC_OSNULL ) )
    {
        ...
        for ( inputStringIndex = 0; 
              inputStringIndex < inputStringLen; 
              inputStringIndex++ ) 
        {
            ospvStringLowercaseOut[ inputStringIndex ] = 
                (char)(OSPM_TOLOWER( ospvStringIn[ inputStringIndex ]) );
        }
    ...
    )
{
    ...
    {
        for ( argvIndex = 1; 
              ( argvIndex < ospvArgc ) && ( retVal == OSPC_ERR_NO_ERROR ); 
              argvIndex++ )
        {
                 ...
                 ( nextArg != OSPC_OSNULL ) )
            {
                if ( argvIndex < ospvArgc - 1 )
                {
                    if ( OSPM_STRCMP( nextArg, OSPC_ENROLL_FUNCTION_PARAM ) == 0 )
                    {
                        ...
                        if ( ( retVal != OSPC_ERR_NO_ERROR ) ||
                             ( enrollParams->Function == OSPC_OSNULL ) )
                        {
                            retVal = OSPC_ERR_ENROLL_PARAMS_FUNCTION;
                            OSPM_DBGERRORLOG( 
                                retVal, 
                                "Unable to copy the operation.\n" );
                        }
                    }
                    ...
                    else if ( OSPM_STRCMP( nextArg, OSPC_ENROLL_USERNAME_PARAM ) == 0 )
                    {
                        retVal = 
                            OSPPCopyString( 
                                &(enrollParams->Username), 
                                (unsigned char *)ospvArgv[ argvIndex + 1 ] );
    
                        if ( ( retVal != OSPC_ERR_NO_ERROR ) ||
                             ( enrollParams->Username == OSPC_OSNULL ) )
                        {
                            retVal = OSPC_ERR_ENROLL_PARAMS_USERNAME;
                            OSPM_DBGERRORLOG( 
                                retVal, 
                                "Unable to copy the username.\n" );
                        }
                    }
                    ...
                    else if ( OSPM_STRCMP( nextArg, OSPC_ENROLL_PASSWORD_PARAM ) == 0 )
                    {
                        retVal = 
                            OSPPCopyString( 
                                &(enrollParams->Password), 
                                (unsigned char *)ospvArgv[ argvIndex + 1 ] );
    
                        if ( ( retVal != OSPC_ERR_NO_ERROR ) ||
                             ( enrollParams->Password == OSPC_OSNULL ) )
                        {
                    ...
                    else if ( OSPM_STRCMP( nextArg, OSPC_ENROLL_DEVICEID_PARAM ) == 0 )
                    {
                        retVal = 
                            OSPPCopyString( 
                                &(enrollParams->DeviceId), 
                                (unsigned char *)ospvArgv[ argvIndex + 1 ] );
    
                        if ( ( retVal != OSPC_ERR_NO_ERROR ) ||
                             ( enrollParams->DeviceId == OSPC_OSNULL ) )
                        {
                            retVal = OSPC_ERR_ENROLL_PARAMS_DEVICEID;
                            OSPM_DBGERRORLOG( 
                                retVal, 
                                "Unable to copy the device id.\n" );
                        }
                    }
                    ...
                    else if ( OSPM_STRCMP( nextArg, OSPC_ENROLL_CUSTOMERID_PARAM ) == 0 )
                    {
                        retVal = 
                            OSPPCopyString( 
                                &(enrollParams->CustomerId), 
                                (unsigned char *)ospvArgv[ argvIndex + 1 ] );
    
                        if ( ( retVal != OSPC_ERR_NO_ERROR ) ||
                             ( enrollParams->CustomerId == OSPC_OSNULL ) )
                        {
                            retVal = OSPC_ERR_ENROLL_PARAMS_CUSTOMERID;
                            OSPM_DBGERRORLOG( 
                                retVal, 
                                "Unable to copy the customer id.\n" );
                        }
                    }
                    ...
                    else if ( OSPM_STRCMP( nextArg, OSPC_ENROLL_CA_URL_PARAM ) == 0 )
                    {
                        ...
                        if ( ( retVal != OSPC_ERR_NO_ERROR ) ||
                             ( enrollParams->CAUrl == OSPC_OSNULL ) )
                        {
                            retVal = OSPC_ERR_ENROLL_PARAMS_CA_URL;
                            OSPM_DBGERRORLOG( 
                                retVal, 
                                "Unable to copy the CA's url.\n" );
                        }
                    }
                    ...
                    else if ( OSPM_STRCMP( nextArg, OSPC_ENROLL_SSL_URL_PARAM ) == 0 )
                    {
                        ...
                        if ( ( retVal != OSPC_ERR_NO_ERROR ) ||
                             ( enrollParams->SSLUrl == OSPC_OSNULL ) )
                        {
                            retVal = OSPC_ERR_ENROLL_PARAMS_SSL_URL;
                            OSPM_DBGERRORLOG( 
                                retVal, 
                                "Unable to copy the enrollment server's url.\n" );
                        }
                    }
                    ...
                    else if ( OSPM_STRCMP( nextArg, OSPC_ENROLL_CA_FPRINT_PARAM ) == 0 )
                    {
                        retVal = 
                            OSPPCopyString( 
                                &(enrollParams->CAFprint), 
                                (unsigned char *)ospvArgv[ argvIndex + 1 ] );
    
                        if ( ( retVal != OSPC_ERR_NO_ERROR ) ||
                             ( enrollParams->CAFprint == OSPC_OSNULL ) )
                        {
                            retVal = OSPC_ERR_ENROLL_PARAMS_CA_FPRINT;
                            OSPM_DBGERRORLOG( 
                                retVal, 
                                "Unable to copy the CA cert's fingerprint.\n" );
                        }
                    }
                ...
                if ( retVal != OSPC_ERR_NO_ERROR )
                {
                    OSPM_DBGERRORLOG( 
                        retVal, 
                        "An invalid enrollment parameter was parsed.\n" );
                }
...
)
{
    ...
    /* If ( the certificate is null or it's empty ) then
     *  o warn the user.
     */
    ...
    if ( retVal == OSPC_ERR_NO_ERROR )
    {
        ...
        retVal =
            OSPPBase64Encode( ospvCert, ospvCertLen, certB64, &certB64Len );
        if ( retVal != OSPC_ERR_NO_ERROR )
        {
...
)
{
     ...
     * if necessary.
     */
    if ( ( ospvCmd != OSPC_OSNULL ) &&
         ( ospvHelpMsg != OSPC_OSNULL ) )
    {
